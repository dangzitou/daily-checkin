...
function toggleRepeatDay(day: number) {
  const idx = taskForm.repeatDays.indexOf(day);
  if (idx >= 0) taskForm.repeatDays.splice(idx, 1);
  else taskForm.repeatDays.push(day);
}
...
async function loadTasks() {
  ...
  try {
    ...
    tasks.value = result.filter((t) => t.scope !== 'dated');
  } catch (err) {
    taskLoadError.value = err instanceof Error ? err.message : '加载任务失败';
  }
}
...
async function createTask() {
  ...
  try {
    ...
    if (taskForm.scope === 'weekly' || taskForm.scope === 'monthly') {
      if (taskForm.repeatDays.length === 0) {
        taskError.value = taskForm.scope === 'weekly' ? '请选择至少一个星期几' : '请选择至少一个日期';
        taskBusy.value = false;
        return;
      }
      body.repeatDays = taskForm.repeatDays;
    }
    if (taskForm.scope === 'dated') {
      if (!taskForm.scheduledDate) {
        taskError.value = '请选择日期';
        taskBusy.value = false;
        return;
      }
      body.scheduledDate = taskForm.scheduledDate;
    }
    if (taskForm.startDate) {
      body.startDate = taskForm.startDate;
    }
...
async function confirmDestructiveAction() {
  ...
  if (target.kind === 'task') {
    try {
      ...
      taskError.value = err instanceof Error ? err.message : '删除失败';
    } finally {
...
function scopeLabel(scope: string): string {
  return { resident: '每天', weekly: '每周', monthly: '每月', dated: '一次性' }[scope] ?? scope;
}

function repeatDaysLabel(scope: string, raw: string | null): string {
  if (!raw) return '';
  try {
    const days: number[] = JSON.parse(raw);
    if (scope === 'weekly') return days.map((d) => '周' + WEEKDAY_LABELS[d]).join('、');
    if (scope === 'monthly') return days.map((d) => d + '号').join('、');
  } catch {}
  return '';
}
</script>
    ...
    <div v-if="activeTab === 'tasks'">
      <div class="manage-layout">
        <section class="manage-panel">
          ...
          <header>
            <h2>新增任务</h2>
            <p>支持每天、每周、每月和一次性任务。</p>
          </header>
          <form class="task-form" @submit.prevent="createTask">
            <label>
              <span>任务类型</span>
              <select v-model="taskForm.scope" class="scope-select">
                <option value="resident">每天任务</option>
                <option value="weekly">每周任务</option>
                <option value="monthly">每月任务</option>
                <option value="dated">一次性任务</option>
              </select>
            </label>
            <label>
              <span>任务名</span>
            ...
            <!-- 周任务：选择星期几 -->
            <div v-if="taskForm.scope === 'weekly'" class="repeat-picker">
              <span class="repeat-label">重复日</span>
              <div class="repeat-chips">
                <button
                  ...
                  type="button"
                  class="repeat-chip"
                  :class="{ active: taskForm.repeatDays.includes(i) }"
            ...
            <!-- 月任务：选择每月几号 -->
            <div v-if="taskForm.scope === 'monthly'" class="repeat-picker">
              <span class="repeat-label">每月日期</span>
              <div class="repeat-chips month-grid">
                <button
                  v-for="d in MONTH_DAYS"
                  :key="d"
                  type="button"
                  class="repeat-chip small"
                  :class="{ active: taskForm.repeatDays.includes(d) }"
                  @click="toggleRepeatDay(d)"
                >{{ d }}</button>
              </div>
            </div>

            <!-- 一次性任务：选择日期 -->
            <label v-if="taskForm.scope === 'dated'">
              <span>执行日期</span>
            ...
            <label v-if="taskForm.scope !== 'dated'">
              <span>开始日期（可选）</span>
              <input v-model="taskForm.startDate" type="date" placeholder="留空立即生效" />
            </label>
          ...
          <section v-else class="manage-list">
            <article v-for="task in tasks" :key="task.id" class="manage-row">
              <div>
                ...
                <span>{{ scopeLabel(task.scope) }}{{ repeatDaysLabel(task.scope, task.repeatDays) ? ' · ' + repeatDaysLabel(task.scope, task.repeatDays) : '' }}{{ task.reminderTime ? ' · 提醒 ' + task.reminderTime : '' }}</span>
              </div>
